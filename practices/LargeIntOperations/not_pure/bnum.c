...
bnum new_bnum(void){
    bnum num = (bnum)malloc(MAX_DIGIT_NUM*sizeof(digit));
    reset_to_zero(num);
    return num;
}
...
void multiply_bnum(const bnum a, const bnum b, bnum res){
    ...
    for(int i=0; i<MAX_DIGIT_NUM; i++){
        ...
        for(int j=0; j<MAX_DIGIT_NUM; j++){
            ...
            tmp = (int)a[i] * (int)b[j] + carry + (int)res[i+j];
            res[i+j] = tmp%10;
            carry = tmp/10;
        }
    }
...
void _operate_with_bnum_template(bnum x, const bnum y, void (*operator)(const bnum, const bnum, bnum)){
    bnum tmp = new_bnum();
    memcpy(tmp, x, MAX_DIGIT_NUM*sizeof(digit));
    operator(tmp, y, x);
    del_bnum(tmp);
}
...
void module_bnum(bnum a, bnum b, bnum res) {
    ...
    if(comp_res==0){
        ...
        memcpy(res, b, MAX_DIGIT_NUM*sizeof(digit));
        return;
    }
